  ...
  <div class="outfit-page">
    ...
    <div class="page-content">
      ...
      <div class="image-section">
        <van-uploader
          v-model="clothesImages"
          :max-count="5"
          multiple
        />
        <div class="tip">可以是衣服全图、全景图，或者打算购买的衣服图片</div>
      </div>

      <div class="section-title">上传个人照片</div>
      <div class="image-section">
        <van-uploader
          v-model="personImages"
          :max-count="3"
          multiple
        />
      </div>
...
const clothesImages = ref([])
const personImages = ref([])
const selectedGalleryImages = ref([])
const resultImages = ref([])

const canGenerate = computed(() => 
  clothesImages.value.length > 0 || personImages.value.length > 0 || selectedGalleryImages.value.length > 0
)
...
async function generate() {
  try {
    ...
    const res = await generateOutfit({
      clothesImages: clothesImages.value,
      personImages: personImages.value,
...
async function saveToGallery() {
  try {
    for (const img of resultImages.value) {
      await addToGallery({ url: img, type: 'outfit' })
    }
    showToast('保存成功')
  } catch (error) {
    showToast('保存失败')
