<template>
  <div
    class="custom-node parallelogram-node"
    :style="nodeStyle"
  ...
  >
    ...
    <div class="node-text" :style="textStyle">{{ data.label || '平行四边形' }}</div>
  </div>
</template>
...
const nodeStyle = computed(() => ({
  backgroundColor: node.value?.data?.bgColor || '#fce7f3',
  borderColor: node.value?.data?.borderColor || '#ec4899',
  borderWidth: '2px',
  borderStyle: 'solid',
  borderRadius: '0',
  width: node.value?.style?.width || '180px',
