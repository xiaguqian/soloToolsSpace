<template>
  <div
    class="custom-node rectangle-node"
    :style="nodeStyle"
  ...
  >
    ...
    <div class="node-text" :style="textStyle">{{ data.label || '矩形节点' }}</div>
  </div>
</template>
...
const nodeStyle = computed(() => ({
  backgroundColor: node.value?.data?.bgColor || '#ffffff',
  borderColor: node.value?.data?.borderColor || '#cbd5e1',
  borderWidth: '2px',
  borderStyle: 'solid',
  borderRadius: '4px',
