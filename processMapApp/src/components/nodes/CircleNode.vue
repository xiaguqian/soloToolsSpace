<template>
  ...
  >
    ...
    <div class="node-text" :style="textStyle">{{ data.label || '圆形节点' }}</div>
  </div>
</template>
...
const nodeStyle = computed(() => ({
  backgroundColor: node.value?.data?.bgColor || '#e0e7ff',
  borderColor: node.value?.data?.borderColor || '#6366f1',
  borderWidth: '2px',
  borderStyle: 'solid',
  borderRadius: '50%',
  width: node.value?.style?.width || '120px',
  height: node.value?.style?.height || '120px',
  display: 'flex',
  alignItems: 'center',
