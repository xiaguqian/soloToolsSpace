<template>
  ...
  >
    ...
    <Handle
      ...
      :style="handleStyle"
    />
    <div class="node-text" :style="textStyle">{{ data.label || '开始' }}</div>
  </div>
</template>
...
const nodeStyle = computed(() => ({
  backgroundColor: node.value?.data?.bgColor || '#10b981',
  borderColor: node.value?.data?.borderColor || '#059669',
  borderWidth: '2px',
  borderStyle: 'solid',
  borderRadius: '50%',
